...
**現在のバージョン: v0.2**（`md.inc.php` 内の `MD_PLUGIN_VERSION` 定数およびファイルヘッダに記載）
...
[こちらのブログ](https://oncologynote.jp/4b3622e9ab)にもこのmd.inc.phpに関する解説記事があります。    
...
//   // キャッシュ時に毎回再実行するブロックプラグイン（カスタマイズ時のみ追記）
```

> **キャッシュとプラグインの相性**
>
> v0.3 以降、`!include`, `!pcomment`, `!recent` 等の**動的ブロックプラグイン**はキャッシュから自動的に除外され、ページ表示のたびに再実行されます。これにより、キャッシュを有効にしたままコメント・最近の更新・外部ページの変更が正しく反映されます。
>
> - **自動除外される動的プラグイン（デフォルト）**: `include`, `pcomment`, `comment`, `recent`, `popular`, `counter`, `online`, `calendar`, `showrss`, `ls`, `ls2`
> - リストは `$markdown_cache_dynamic_plugins` でカスタマイズ可能
> - **対象はブロックプラグイン（`!plugin(args)` 形式）のみ**。`&counter;` 等のインラインプラグインはキャッシュに含まれたままです
>
> 性能面では、CommonMark パース（重い処理）はキャッシュされ、軽量なプラグイン実行のみ毎回行われるため、80〜95% のキャッシュ効果が維持されます。
...
- **v0.3**（2026-06-17）: 動的ブロックプラグイン（`include`, `pcomment`, `recent` 等）をキャッシュから除外し毎回再実行するプレースホルダー方式を導入。キャッシュ有効時でもコメント・最近の更新等が正しく反映される
- **v0.2**（2026-06-17）: 本番版（oncologynote.jp）の改良を導入。GFMバンドルを個別拡張に分割（プラグイン出力の `<textarea>` 等のエスケープ問題を回避）、例外処理を Throwable に変更（Fatalエラーも捕捉）
